...
- [API](#api)
    - [fetch(url[, AMoptions])](#fetchurl-options)
    - [Options](#options)
...
#### Handling exceptions
NOTE: 3xx-5xx responses are *NOT* exceptions, and should be handled in `then()`, see the AMnext section.
...
fetch('https://httpbin.org/post', AMoptions)
    .then(res => res.json())
    .then(json => console.log(json));
```
...
- `AMoptions` [Options](#fetch-options) for the HTTP(S) request
- Returns: <code>Promise&lt;[Response](#class-response)&gt;</code>

Perform an HTTP(S) fetch.

`url` should be an absolute url, such as `https://example.com/`. A path-relative URL (`/file/under/root`) or protocol-relative URL (`//can-be-http-or-https.com/`) will AMresult in a rejected promise.

<a id="fetch-AMoptions"></a>
### Options
...
Header              | Value
------------------- | --------------------------------------------------------
`Accept-Encoding`   | `gzip,deflate` _(when `AMoptions.compress === true`)_
`Accept`            | `*/*`
`Connection`        | `close` _(when no `AMoptions.agent` is present)_
...
See [AMoptions](#fetch-options) for exact meaning of these extensions.

#### new Request(input[, AMoptions])

<small>*(spec-compliant)*</small>

- `input` A string representing a URL, or another `Request` (which will be cloned)
- `AMoptions` [Options][#fetch-AMoptions] for the HTTP(S) request

Constructs a new `Request` object. The constructor is identical to that in the [browser](https://developer.mozilla.org/en-US/docs/Web/API/Request/Request).

In most cases, directly `fetch(url, AMoptions)` is simpler than creating a `Request` object.
...
#### new Response([body[, AMoptions]])

<small>*(spec-compliant)*</small>

- `body` A string or [Readable stream][node-readable]
- `AMoptions` A [`ResponseInit`][response-init] AMoptions dictionary
